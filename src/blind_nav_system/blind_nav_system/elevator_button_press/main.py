...
# 오늘 실기의 스텝별 '명령 vs odom이동' 실측 초과가 0.3~1.5cm였다. 1.0으로 두면
# 목표를 넘은 만큼 되돌리는 스텝이 계속 나가 SCENE_LEG_MAX_STEPS(30)를 갈아먹는다.
...
# _scene_leg 가 SCENE_TOL_* 에서 "다 맞췄다"며 멈추므로, 종료 조건이 그보다 엄하면
# 완료가 원리적으로 불가능하다 — 정렬은 성공인데 로그만 '⚠ 미달'로 찍힌다.
# (2026-09-04에 회전 쪽이 실제로 뒤집혔다: 스텝 문턱만 3.0→5.0으로 올리고 종료
#  조건은 3.0으로 남겨 3~5°로 끝나면 미달로 보고됐다.)
# 그래서 값을 따로 적지 않고 구간 허용오차에서 파생시킨다 — 앞으로 어느 한쪽만
# 바뀌어도 이 관계가 깨지지 않는다. 앞의 상수(0.05m / 3.0°)가 원래 목표 정밀도이고,
# 구간 허용오차가 그보다 느슨해지면 느슨한 쪽을 따라간다.
# 지금 값: 거리 max(0.05, 0.01) = 0.05m — 오늘 기준 변화 없음(거리 쌍은 원래 정상)
#          자세 max(3.0, 5.0)    = 5.0°  — 3.0에서 완화(오늘 고친 것)
# 거리 0.05m 근거: 1cm는 AMCL로 도달 불가능해 불필요한 반복만 유발한다.
#                  Nav2의 xy_goal_tolerance도 0.05다.
POSE_DONE_M   = max(0.05, SCENE_TOL_FWD_CM / 100.0)
POSE_DONE_DEG = max(3.0,  SCENE_TOL_ROT_DEG)

# 남은 거리가 이 이하면 ①②를 건너뛰고 ③정렬만 한다.
# 2026-09-08: 0.20 → POSE_DONE_M 파생(= 0.05). 0.20은 사각지대를 만들었다 —
# 0.05 < dist < 0.20 구간에서 위치를 줄일 수단이 아예 없어 ③정렬만 반복하다
...
# 그 사이 구간은 "종료도 못 하고 고치지도 못하는" 상태가 된다.
# 반대로 이보다 더 내려서도 안 된다: 최상단 종료가
# `dist <= POSE_DONE_M and |rot| <= POSE_DONE_DEG` 인 AND 라서, dist 3cm 여도 yaw 가
# 틀어져 있으면 종료하지 않고 이 검사로 내려온다. SKIP 이 0.02 면 그 3cm 오프셋에
# 대한 방위(잡음 지배)로 ①정렬이 크게 돈다 — 2026-09-04에 겪은 그 실패다.
# 같게 두면 그 구간이 ③정렬만 도는 정상 경로가 된다.
POSE_SKIP_M   = POSE_DONE_M
