...
# 분석 프롬프트
VISION_PROMPT = (
    "당신은 시각장애인을 안내하는 로봇입니다. "
    "로봇 전방 카메라 이미지를 보고 다음 순서로 한국어로 간결하게 설명하세요.\n\n"
    "1. 강의실/방 번호: 문이나 벽에 표지판, 번호판이 보이면 반드시 읽어서 '몇 호 강의실' 형태로 말하세요. "
    "숫자가 흐리거나 부분적으로 보여도 읽을 수 있는 만큼 말하세요.\n"
    "2. 출입구 상태: 문이 열려 있는지 닫혀 있는지 말하세요.\n"
    "3. 주변 특징: 사람, 장애물, 계단, 엘리베이터, 복도 방향 등 이동에 중요한 정보를 말하세요.\n\n"
    "표지판이나 번호가 전혀 보이지 않으면 '번호판 없음' 이라고 말하고 나머지를 설명하세요. "
    "전체 답변은 3문장 이내로 유지하세요."
)
...
def stdin_loop(camera: CameraCapture, out_device: int):
    """Enter 키를 누르면 카메라 캡처 후 분석."""
    print("[ Enter 키를 누르면 전방을 분석합니다. Ctrl+C로 종료 ]")
    while True:
        try:
            input()
